...
def drill_survy_calculation():
    ...
    def tengen():
        ...
        n1 = []
        e1 = []
        Md1 = float(Entry.get(E1))
        Md2 = float(Entry.get(E2))
        Md3 = float(Entry.get(E3))
        Md4 = float(Entry.get(E4))
        Md5 = float(Entry.get(E5))
        Md6 = float(Entry.get(E6))
        ##==============================================================
        inc1 = float(Entry.get(E7))
        inc2 = float(Entry.get(E8))
        inc3 = float(Entry.get(E9))
        inc4 = float(Entry.get(E10))
        inc5 = float(Entry.get(E11))
        ##============================================================================
        azi1 = float(Entry.get(E12))
        azi2 = float(Entry.get(E13))
        azi3 = float(Entry.get(E14))
        azi4 = float(Entry.get(E15))
        azi5 = float(Entry.get(E16))
        kop = Md1
        d = [Md2, Md3, Md4, Md5, Md6]
        l1 = Md3 - Md2
        l2 = Md4 - Md3
        l3 = Md5 - Md4
        l4 = Md6 - Md5
        phi1 = float((inc1 * 100) / (Md2 - Md1))

        r = 18000 / (math.pi * phi1)
        V1 = kop + r * math.sin(np.radians(inc1))
        H1 = r * (1 - math.cos(np.radians(inc1)))
        N1 = H1 * math.cos(np.radians(azi1))
        Ee1 = H1 * math.sin(np.radians(azi1))
        len = [l1, l2, l3, l4]
        inc = [inc1, inc2, inc3, inc4, inc5]
        azi = [azi1, azi2, azi3, azi4, azi5]
        x = 1
        for i in len:
            delv = i * math.cos(np.radians(inc[x]))
            delh = i * math.sin(np.radians(inc[x]))
            deln = delh * math.cos(np.radians(azi[x]))
            dele = delh * math.sin(np.radians(azi[x]))
            x = x + 1
    ...
    L9 = Label(pro2, text="inc1", ).grid(row=1, column=2)
    L10 = Label(pro2, text="inc2", ).grid(row=2, column=2)
    L11 = Label(pro2, text="inc3", ).grid(row=3, column=2)
    L12 = Label(pro2, text="inc4", ).grid(row=4, column=2)
    L13 = Label(pro2, text="inc5", ).grid(row=5, column=2)
    ...
    L14 = Label(pro2, text="azi1", ).grid(row=1, column=4)
    L15 = Label(pro2, text="azi2", ).grid(row=2, column=4)
    L16 = Label(pro2, text="azi3", ).grid(row=3, column=4)
    L17 = Label(pro2, text="azi4", ).grid(row=4, column=4)
    L18 = Label(pro2, text="azi5", ).grid(row=5, column=4)
...
def proces1():
    ...
    L2 = Label(top, text="Xs", ).grid(row=1, column=0)
    L3 = Label(top, text="Xt", ).grid(row=2, column=0)
    L4 = Label(top, text="Ys", ).grid(row=3, column=0)
    L5 = Label(top, text="Yt", ).grid(row=4, column=0)
    ...
    def getData():
        ...
        MDb = Vb
        R = roc(phi)
        Ht, azi = targetHandBeta(Et, Es, Nt, Ns)
        if Ht>R:
            omega = round(np.degrees(np.arctan((Vt - Vb) / (Ht - R))), 2)
            delta = round(np.degrees(np.arccos(R / ((Ht - R) ** 2 + (Vt - Vb) ** 2) ** .5)))
            beta = 180-omega-delta
            Vc = round(Vb + (R * math.sin(np.radians(beta))), 2)
            Hc = round(R * (1 - math.cos(np.radians(beta))), 2)
            MDc = round(Vb + ((beta / phi) * 100), 2)
            MDt = round(MDc + (Vt - Vc) / math.cos(math.radians(beta)), 2)

        else:
            omega = round(np.degrees(np.arctan((R-Ht) / (Vt-Vb))), 2)
            delta = round(np.degrees(np.arccos(R / ((Ht - R) ** 2 + (Vt - Vb) ** 2) ** .5)))
            beta= 90- omega - delta
            Vc = round(Vb + (R * math.cos(np.radians(90-beta))), 2)
            Hc = round(R * (1 - math.sin(np.radians(90-beta))), 2)
            MDc = round(Vb + ((beta / phi) * 100), 2)
            MDt = round(MDc + (Vt - Vc) / math.cos(math.radians(beta)), 2)


        return Ns, Es, Vb, Hb, MDb, Vt, phi, R, Ht, azi, beta, Vc, Hc, MDt, MDc

    def calcTrajectory(measuredDepth):
        Ns, Es, Vb, Hb, MDb, Vt, phi, R, Ht, azi, beta, Vc, Hc, MDt, MDc = getData()
        h = []
        v = []
        nx = []
        ex = []
    ...
    def showgraph():
        ...
        a.text(Hc + 50, np.negative(Vc), "(C) Build off point", **style)
        a.text(Ht - 55, np.negative(Vt), "Target point (T)", **style)
    ...
    def show3d():
        ...
        Ha = 0
        Va = 0
        Nb = round(Ns + Hb * math.cos(azi), 2)
        Eb = round(Es + Hb * math.sin(azi), 2)
        Nc = round(Ns + Hc * math.cos(azi), 2)
        Ec = round(Es + Hc * math.sin(azi), 2)
        Nt = round(Ns + Ht * math.cos(azi), 2)
        Et = round(Es + Ht * math.sin(azi), 2)
        style = dict(size=7, color='black')
        ax.text(Ns, Es, Va, "(A) Well Head", **style)
        ax.text(Nb, Eb, Vb, "(B) Kick off point", **style)
        ax.text(Nc, Ec, Vc, "(C) Build off point", **style)
        ax.text(Nt, Et, Vt, "Target point (T)", **style)
        ax.tick_params(axis="x", labelsize=8)
        ax.tick_params(axis="y", labelsize=8)
        ax.tick_params(axis="z", labelsize=8)
    ...
    def proces3():
        ...
        def final():
            ...
            finaldata.to_csv(r"C:\Users\ri\OneDrive\Desktop\well profile\Profile_1.csv", index_label='Points')
...
def proces5():
    ...
    L17 = Label(top, text="Ns", ).grid(row=1, column=0)
    ...
    L4 = Label(top, text="phi1", ).grid(row=6, column=0)
    L5 = Label(top, text="phi2", ).grid(row=7, column=0)
    L6 = Label(top, text="Vb", ).grid(row=8, column=0)
    L7 = Label(top, text="Alpha2", ).grid(row=9, column=0)
    # l9 = Label(top, text="Press Submit...", ).grid(row=8, column=1)
    L8 = Label(top, text="A", ).grid(row=1, column=2)
    L9 = Label(top, text="B", ).grid(row=2, column=2)
    L10 = Label(top, text="C", ).grid(row=3, column=2)
    L11 = Label(top, text="D", ).grid(row=4, column=2)
    L12 = Label(top, text="T", ).grid(row=5, column=2)


    L13 = Label(top, text="Enter Values Here").grid(row=0, column=1)

    L14 = Label(top, text="V", ).grid(row=0, column=3)
    L15 = Label(top, text="H", ).grid(row=0, column=4)
    L16 = Label(top, text="MD", ).grid(row=0, column=5)

    z7 = Entry(top, bd=5)
    z7.grid(row=1, column=1)
    z8 = Entry(top, bd=5)
    z8.grid(row=2, column=1)
    z9 = Entry(top, bd=5)
    z9.grid(row=3, column=1)
    z10 = Entry(top, bd=5)
    z10.grid(row=4, column=1)
    ...
    def getData():
        Ns = float(Entry.get(z7))
        Es = float(Entry.get(z8))
        Nt = float(Entry.get(z9))
        Et = float(Entry.get(z10))
        # Ht = float(Entry.get(z1))
        Vt = float(Entry.get(z2))
        phi1 = float(Entry.get(z3))
        phi2 = float(Entry.get(z4))
        Vb = float(Entry.get(z5))
        alpha2 = np.radians(float(Entry.get(z6)))
        Ht,azi = targetHandBeta(Et, Es, Nt, Ns)
        R1 = roc(phi1)
        R2 = roc(phi2)
        if Ht>R1 and Ht>R2:
            D1 = R1*(1-math.cos(alpha2))
            X1 = Ht-D1-R2
            D2 = D1+X1+R2*math.cos(alpha2)
            beta = np.degrees(np.arctan((X1 + R2 * math.cos(alpha2)) / (R1 * math.sin(alpha2))))
            alpha = np.degrees(np.arctan((Vt - Vb) / (Ht - R1)))
            delta = np.degrees(math.pi+alpha2 - np.radians(beta + alpha))

            print((alpha))
        # alpha1Deg = math.degrees(alpha1)
            Vc = round(Vb + R1 * math.sin(alpha2), 2)
            Hc = round(R1 * (1 - math.cos(alpha2)), 2)
            MDc = round((alpha2) / (phi1) * 100 + Vb, 2)
            Vd = round(Vc + (X1+R2*math.cos(alpha2))/(math.tan(alpha2)), 2)
            Hd = round(Hc + (X1+R2*math.cos(alpha2)), 2)
            MDd = round(MDc +(X1+R2*math.cos(alpha2))/(math.cos(alpha2)), 2)
            MDt = round(MDd + (alpha2) / (phi2) * 100, 2)

        return Ht, Vt, phi1, phi2, Vb, alpha2, R1, R2,Vc, Vd, Hc, Hd,MDc,MDd, MDt, beta,azi, Ns, Nt, Es, Et

    def calcTrajectory(measuredDepth):
        Ht, Vt, phi1, phi2, Vb, alpha2, R1, R2,Vc, Vd, Hc, Hd,MDc,MDd, MDt, beta,azi, Ns, Nt, Es, Et = getData()
        h = []
        v = []
        nx = []
        ex = []
        ...
        for d in measuredDepth:
            if d <= Vb:
                ...
                Nx = round(Ns + Hx * math.cos(azi), 2)
                Ex = round(Es + Hx * math.sin(azi), 2)
                v.append(Vx)
                h.append(Hx)
                nx.append(Nx)
                ex.append(Ex)
                ...
                L = d - MDc
                # y = alpha1
                vx = round(Vc + L * math.cos(alpha2), 2)
                hx = round(Hc + L * math.sin(alpha2), 2)
                Nx = round(Ns + hx * math.cos(azi), 2)
                Ex = round(Es + hx * math.sin(azi), 2)
                v.append(vx)
                h.append(hx)
            ...
            elif MDd < d and d <= MDt:  # dropup
                L = d - MDd
                z = math.radians(L * phi2 / 100)
                vx = round(Vd + R2 * (math.sin(alpha2) - math.sin(alpha2 - z)), 2)
                hx = round(Hd + R2 * (math.cos(alpha2 - z) - math.cos(alpha2)), 2)
                Nx = round(Ns + hx * math.cos(azi), 2)
                Ex = round(Es + hx * math.sin(azi), 2)
                v.append(vx)
                h.append(hx)
    ...
    def process2():
        ...
        L31 = Label(top, text=str(Vt)).grid(row=5, column=3)
        L32 = Label(top, text=str(Ht)).grid(row=5, column=4)
        L33 = Label(top, text=str(MDt)).grid(row=5, column=5)
    ...
    def showgraph():
        ...
        a1.text(Hc, np.negative(Vc), "(C) Build off point", **style)
        a1.text(Hd, np.negative(Vd), "(D) Drop up point", **style)
        a1.text(Ht, np.negative(Vt), "Target point (T)", **style)
    ...
    def show3d():
        ...
        Nc = round(Ns + Hc * math.cos(azi), 2)
        Ec = round(Es + Hc * math.sin(azi), 2)
        Nd = round(Ns + Hd * math.cos(azi), 2)
        Ed = round(Es + Hd * math.sin(azi), 2)
        Nt = round(Ns + Ht * math.cos(azi), 2)
        Et = round(Es + Ht * math.sin(azi), 2)
        style = dict(size=7, color='black')
        ax.text(Ns, Es, Va, "(A) Well Head", **style)
    ...
    def process3():
        ...
        def final():
            ...
            finaldata = DataFrame(data, columns=['V', 'H', 'MD', '', 'Entered MD', 'V', 'H', 'N', 'E'],
                                  index=['A', 'B', 'C', 'D', 'T', ''])
            # np.savetxt('C:\Users\sk314\Documents\Profile_1_text.txt', data, delimiter=',')
            finaldata.to_csv(r'C:\Users\ri\OneDrive\Desktop\well profile\Profile_2.csv', index_label='Points')
    ...
    def proces3():
        ...
        def final():
            ...
            finaldata = DataFrame(data, columns=['V', 'H', 'MD', '', 'Entered MD', 'V', 'H'],
                                  index=['A', 'B', 'C', 'T', '', '', ''])
            finaldata.to_csv(r'C:\Users\ri\OneDrive\Desktop\well profile\Profile_4.csv', index_label='Points')
            L40 = Label(pro2, text="Data has been Saved\n Check Document File").grid(row=13, column=1)
            print('finaldata')
            print(finaldata)
...
def proces9():
    ...
    def proces3():
        ...
        def final():
            ...
            finaldata = DataFrame(data, columns=['V', 'H', 'MD', '', 'Entered MD', 'V', 'H', 'N', 'E'],
                                  index=['A', 'B', 'C', 'D', 'E', 'T', ''])
            finaldata.to_csv('C:/Users/ri/OneDrive/Desktop/well profile/Profile_5B.csv', index_label='Points')
            L40 = Label(pro2, text="Data has been Saved\n Check Document File").grid(row=13, column=1)
            print('finaldata')
            print(finaldata)
...
def proces11():
    ...
    def proces3():
        ...
        def final():
            ...
            finaldata = DataFrame(data, columns=['V', 'H', 'MD', '', 'Entered MD', 'V', 'H', 'N', 'E'],
                                  index=['A', 'B', 'C', 'T', '', '', ''])
            finaldata.to_csv(r'C:\Users\ri\OneDrive\Desktop\well profile\Profile_5A.csv', index_label='Points')
            L40 = Label(pro2, text="Data has been Saved\n Check Document File").grid(row=13, column=1)
...
def comp():
    ...
    def f1():
        ...
        Entry.insert(eh10, 0, t[2])
        Entry.insert(eh11, 0, t[3])
        Entry.insert(eh12, 0, t[4])
    ...
    kr = Label(top, text="Pro3", ).grid(row=0, column=6)
    ...
    Lk5 = Label(top, text="Pro4A", ).grid(row=0, column=8)
    el1 = Entry(top, bd=5)
    ...
    ldr = Label(top, text="Pro4B", ).grid(row=0, column=10)
    ...
    lh4 = Label(top, text="Pro3", ).grid(row=3, column=12)
    lh5 = Label(top, text="Pro4A", ).grid(row=4, column=12)
    lh6 = Label(top, text="Pro4B", ).grid(row=5, column=12)
    ...
    B = Button(top, text="Submit", command=f1, activebackground="grey", activeforeground="yellow").grid(row=10,
                                                                                                        column=13)
    top.mainloop()
...
def compb():
    ...
    top.title("Well Log File Input")

    def open_file_dialog():
        file_path = filedialog.askopenfilename(initialdir=os.path.expanduser('~\\Downloads'),
                                               filetypes=[("Log Files", "*.las")])
        if file_path:
            getData(file_path)
    ...
    def display_log_file(file_path):
        with open(file_path, 'r') as file:
            ...
            print(log_contents)
            text_widget.delete(1.0, top.END)  # Clear existing text
            text_widget.insert(top.END, log_contents)

    L1 = Label(top, text="TYPE optimize\n PROFILE", bd=7, relief=RAISED, padx=20, width=10).grid(row=0, column=0)
    L2 = Label(top, text="Ns", ).grid(row=1, column=0)
    L3 = Label(top, text="Es", ).grid(row=2, column=0)
    L4 = Label(top, text="Nt", ).grid(row=3, column=0)
    L5 = Label(top, text="Et", ).grid(row=4, column=0)
    L6 = Label(top, text="Enter Values Here").grid(row=0, column=1)
    e1 = Entry(top, bd=5)
    e1.grid(row=1, column=1)
    e2 = Entry(top, bd=5)
    e2.grid(row=2, column=1)
    ...
    l7 = Label(top, text="las file", font=('arial', 10, 'bold'))
    l7.grid(row=1, column=2, ipadx=70, ipady=20)

    b2 = Button(top, command=open_file_dialog, text="Browse")
    ...
    def getData(file_path):
        ...
        if not all(value.strip() and value.replace(".", "", 1).isdigit() for value in
                   [ns_value, nt_value, es_value, et_value]):
            messagebox.showerror("Input Error", "Please enter valid numeric values in all fields.")
            return

        # Convert the values to floats after input validation
        Ns = float(ns_value)
        Nt = float(nt_value)
        Es = float(es_value)
        Et = float(et_value)
...
def help():
    help = Tk()
    h = Label(help, text="RITESH and ANURAG is working on these calculation. Please try after some time.", width=300, height=300,
              font=('arial 20 bold')).pack()
    help.mainloop()
...
b4 = Button(f2, command=help, text="PRESS TO SELECT TYPE 4").grid(row=2, column=2)

l7 = Label(f2, text="TYPE 5(A) PROFILE", font=('arial', 10, 'bold'))
l7.grid(row=3, column=2, ipadx=120, ipady=20)
b5 = Button(f2, command=help, text="PRESS TO SELECT TYPE 5(A)").grid(row=4, column=2)

l8 = Label(f2, text="TYPE 5(B) PROFILE", font=('arial', 10, 'bold'))
l8.grid(row=5, column=2, ipadx=120, ipady=20)
b6 = Button(f2, command=help, text="PRESS TO SELECT TYPE 5(B)").grid(row=6, column=2)
l11 = Label(f2, text="OPTIMIZATION", font=('arial', 10, 'bold'))
l11.grid(row=3, column=3, ipady=10)
b = Button(f2, command=compb, text="Press to compare the well profiles Part 1 ").grid(row=4, column=3, pady=10)
p = Button(f2, command=compa, text="Press to compare the well profiles Part 2").grid(row=5, column=3, pady=10, padx=10)
l2 = Label(bot, text="DIRECTIONAL SURVEYING", font=('arial', 15, 'bold'))
l2.grid(row=0, column=1, pady=40)
b6 = Button(bot, command=drill_survy_calculation,
            text="Press to find the Change in Trajectory at different points").grid(row=1, column=1, pady=10, padx=20)
